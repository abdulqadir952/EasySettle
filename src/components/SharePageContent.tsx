    ...
    if (tripId) {
      const fetchTrip = async () => {
        try {
          ...
          if (tripSnap.exists()) {
            setTrip({ ...tripSnap.data(), id: tripSnap.id } as Trip);
          } else {
            setError("The shared trip could not be found.");
          }
        } catch (e) {
          console.error("Failed to fetch trip data from Firestore", e);
      ...
      setError("No trip ID found in the link.");
      setLoading(false);
    }
  }, [tripId]);
      ...
      <div className="flex flex-col items-center justify-center min-h-screen bg-secondary p-4 text-center">
        <Card className="max-w-md w-full">
          <CardHeader>
            ...
            <CardDescription>
              {error || "The trip you are looking for does not exist or the link is invalid."}
            </CardDescription>
          </CardHeader>
          <CardContent>
            <Button asChild>
